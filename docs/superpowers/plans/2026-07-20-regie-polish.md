...
- **Audio playback + its IPC notify bridge currently live only inside `RegieScreen`'s live-session branch, and this task must fix that.** Flagged by the nav-shell-regie-v1 plan's final whole-branch review: `AudioController` (and the `useAudios`-driven `audio-play`→`electronAPI.trackPlay`/`trackEnd` notify effect it owns) is only mounted when a session is running AND the Régie screen is the active screen. Once this plan wires `RegieTrackPicker`'s "Démarrer" button to dispatch a real `audio-play` event, that means: navigating away from Régie (or a session ending) unmounts `AudioController` mid-playback, silently stopping the audio without ever dispatching a matching `audio-end` — leaving the Sidebar's music pastille stuck on, and the main process never told the track stopped. Before or alongside wiring `RegieTrackPicker` into `RegieScreen`, hoist `<AudioController/>` (and its notify effect) to always be mounted regardless of which screen or session state is active — e.g. render it once in `Dashboard.js` alongside `Sidebar`, not inside `RegieScreen`'s conditional branches — so playback and its IPC bridge survive navigation and session lifecycle.
